...
void CheckBoundaryT1s(const Segment& first_seg, const Segment& second_seg, double& curr_min) {
  ...
  Vector b = Vector(second_seg); // второй направл€ющий вектор
  double t2;
...
void CheckBoundaryT2s(const Segment& first_seg, const Segment& second_seg, double& curr_min) {
  ...
  Vector a = Vector(first_seg); // первый напрвл€ющий вектор
  double t1;
...
double SegmentDistance(const Segment& first_seg, const Segment& second_seg) {
  ...
  double system_answer = SystemSolve(r01, a, r02, b); // находим минимальное рассто€ние через поиск экстремумов функции len(dr), где dr = r01 + t1 * a - r02 - t2 * b
  if (system_answer != -1) {
